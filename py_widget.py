...
Window geometry (size + position) is saved to
~/.config/py_habits_widget/window_geometry.json and restored on next launch.
The grid cells resize dynamically as the window is resized.
"""
import sys
import os
import json
...
GEOMETRY_FILE = os.path.expanduser('~/.config/py_habits_widget/window_geometry.json')

# Minimum cell size so the widget stays usable when shrunk
MIN_CELL_SIZE = 40
# Grid spacing and margins (px)
GRID_SPACING = 3
GRID_MARGIN = 4
...
def load_window_geometry() -> dict:
    """Load saved window geometry. Returns empty dict if not found."""
    try:
...
def compute_cell_size(window_width: int) -> int:
    """
    Compute the cell size that fills the window width across GRID_COLUMNS cells.
    Formula: (width - 2*margin - (cols-1)*spacing) / cols
    """
    available = window_width - 2 * GRID_MARGIN - (GRID_COLUMNS - 1) * GRID_SPACING
    size = available // GRID_COLUMNS
...
class HabitGridWidget(QWidget):
    ...
    The grid cells resize dynamically with the window.
    """
    ...
    def __init__(self):
        ...
        # Minimum window size: 8 cells at MIN_CELL_SIZE + spacing/margins
        min_w = MIN_CELL_SIZE * GRID_COLUMNS + 2 * GRID_MARGIN + (GRID_COLUMNS - 1) * GRID_SPACING
        self.setMinimumSize(min_w, min_w + 80)
    ...
    def _restore_geometry(self):
        """Restore window size and position from saved geometry."""
        geo = load_window_geometry()
        if geo:
            self.resize(geo.get('width', CELL_SIZE * 8 + 80),
                        geo.get('height', CELL_SIZE * 10 + 160))
            screen = QApplication.primaryScreen().availableGeometry()
            x = geo.get('x', (screen.width() - self.width()) // 2)
            y = geo.get('y', (screen.height() - self.height()) // 2)
            # Clamp to screen bounds so the window is never off-screen
            ...
            screen = QApplication.primaryScreen().availableGeometry()
            default_w = CELL_SIZE * 8 + 2 * GRID_MARGIN + (GRID_COLUMNS - 1) * GRID_SPACING
            default_h = CELL_SIZE * 10 + 160
            self.resize(default_w, default_h)
            self.move((screen.width() - default_w) // 2,
    ...
    def closeEvent(self, event):
        ...
        pos = self.pos()
        size = self.size()
        save_window_geometry(pos.x(), pos.y(), size.width(), size.height())
        super().closeEvent(event)

    # ── Resize handling ────────────────────────────────────────────────────────

    def resizeEvent(self, event):
        """Debounce resize events to avoid rebuilding the grid on every pixel."""
        super().resizeEvent(event)
        self._resize_timer.start(80)  # wait 80ms after last resize event

    def _on_resize_settled(self):
        """Called after the window has stopped being resized."""
        new_cell_size = compute_cell_size(self.width())
        if new_cell_size != self._cell_size:
    ...
    def _build_ui(self):
        ...
        # 5. Graphs panel (hidden by default)
    ...
    def _create_habit_cell(self, habit: Habit, index: int, is_move_pending: bool) -> HabitButton:
        ...
        btn = HabitButton(habit, cell_size=self._cell_size)
    ...
    def _create_placeholder_cell(self, index: int, is_move_pending: bool) -> PlaceholderCell:
        ...
        cell = PlaceholderCell(cell_size=self._cell_size)
        cell.set_state(
